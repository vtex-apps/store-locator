...
export function addPickUpPoint() {
    ...
    cy.get(storeLocatorSelectors.PickUpPointName).clear().type("saravanan saravanan")
    cy.get(storeLocatorSelectors.PickUpPointId).type("1")
    cy.get('select').select('United States of America').should('have.value', 'USA')
    cy.wait(1000)
    cy.get(storeLocatorSelectors.PickUpAddress).type('33301 Old Hempstead Rd Magnolia', { delay: 50 })
        .wait(500)
    ...
    cy.get(storeLocatorSelectors.ChangesSaved).should('be.visible').contains('Changes saved')
}
